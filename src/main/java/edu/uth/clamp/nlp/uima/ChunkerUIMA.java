...
public class ChunkerUIMA extends DocProcessor {
	...
	public ChunkerUIMA( Chunker chunker ) {
		...
		while( iter.hasNext() ) {
			...
			while( i < bios.length ) {
				...
				if( bio.equals( CHUNKO ) ) {
					...
					chunk.setBegin( baseTokenList.get(i).getBegin() );
					chunk.setEnd( baseTokenList.get(i).getEnd() );	
					...
					Chunk chunk = new Chunk( aJCas );
					chunk.setBegin( baseTokenList.get(start).getBegin() );
					chunk.setEnd( baseTokenList.get(end).getEnd() );	
