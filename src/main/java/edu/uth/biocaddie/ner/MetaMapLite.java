 ...
 * 		.instantiateBioCDocument(&quot;FDA has strengthened the warning ...&quot;);
 * List&lt;BioCDocument&gt; documentList = new ArrayList&lt;BioCDocument&gt;();
 * documentList.add(document);
 * List&lt;Entity&gt; entityList = metaMapLiteInst.processDocumentList(documentList);
 * for (Entity entity : entityList) {
 * 	for (Ev ev : entity.getEvSet()) {
 * 		System.out.print(ev.getConceptInfo().getCUI() + &quot;|&quot;
 ...
 * <dd>path for part-of-speech model (default: data/models/en-pos-maxent.bin)</dd>
 ...
 * <dd>load configuration from file (default: ./config/metamaplite.properties)</dd>
 * <dt></dt>
 * <dd></dd>
 * </dl>
 * <p>
 ...
 * <dl>
 * <dt>metamaplite.document.inputtype</dt>
 * <dd>set input type for document reader</dd>
 ...
 * <dd>output format of entity result set.</dd>
 ...
 * <dl>
 * <dt>bioc.document.loader.{name}
 * <dt>
 * <dd>classname</dd>
 * </dl>
 * The class must implement the
 * gov.nih.nlm.nls.metamap.document.BioCDocumentLoader interface.
 * <dl>
 * <dt>bioc.document.loader.freetext</dt>
 * <dd>gov.nih.nlm.nls.metamap.document.FreeText</dd>
 * <dt>bioc.document.loader.chemdner</dt>
 * <dd>gov.nih.nlm.nls.metamap.document.ChemDNER</dd>
 * </dl>
 * <p>
 ...
 * <dl>
 * <dt>metamaplite.result.formatter.{name}
 * <dt>
 * <dd>classname</dd>
 * </dl>
 ...
 * <dd>gov.nih.nlm.nls.metamap.lite.resultformats.CuiList</dd>
 * <dt>metamaplite.result.formatter.brat</dt>
 * <dd>gov.nih.nlm.nls.metamap.lite.resultformats.Brat</dd>
 * </dl>
 */
public class MetaMapLite {
	/** log4j logger instance */
	static MetaMapLite instance = null;
	private static final Logger logger = LogManager
			.getLogger(MetaMapLite.class);
	/** location of metamaplite.properties configuration file */
	static String configPropertyFilename = System.getProperty(
			"metamaplite.property.file", "config/metamaplite.properties");
	...
	static public MetaMapLite getDefault() throws IOException,
			ClassNotFoundException, InstantiationException,
			NoSuchMethodException, IllegalAccessException {
		if (instance == null) {
			Properties myProperties = MetaMapLite.getDefaultConfiguration();
			MetaMapLite.expandModelsDir(myProperties, Config.HomeDir
					+ "metamap_data/models");
			MetaMapLite.expandIndexDir(myProperties, Config.HomeDir
					+ "metamap_data/ivf/strict");

			myProperties.setProperty("metamaplite.excluded.termsfile",
					Config.HomeDir + "metamap_data/specialterms.txt");
			// enable cache for speed performance
			if (Config.MetaMapCache) {
				...
				myProperties
						.setProperty(
								...
								"true");
				myProperties.setProperty(
						"metamaplite.normalized.string.cache.enable", "true");
				myProperties.setProperty(
						"metamaplite.entitylookup4.term.concept.cache.size",
						Config.MetaMapCacheSize);
				myProperties
						.setProperty(
								"metamaplite.entitylookup4.cui.preferredname.cache.size",
								Config.MetaMapCacheSize);
				myProperties.setProperty(
						"metamaplite.normalized.string.cache.size", Config.MetaMapCacheSize);
			}
			instance = new MetaMapLite(myProperties);
			instance.setSourceSet(myProperties.getProperty(
					"metamaplite.sourceset", "MSH").split(",")); // /change
																	// there to
																	// MSH
	...
	public MetaMapLite(Properties properties) throws ClassNotFoundException,
			InstantiationException, NoSuchMethodException,
			IllegalAccessException, IOException {
		this.properties = properties;
		if (properties.getProperty("opennlp.en-sent.bin.path") != null) {
			SentenceExtractor.setModel(properties
					.getProperty("opennlp.en-sent.bin.path"));
		}
		this.sentenceAnnotator = new SentenceAnnotator(properties);
		this.entityLookup = new EntityLookup4(properties);
		BioCDocumentLoaderRegistry.register("bioc", "For BioC XML documents.",
				new FreeText());
		...
		BioCDocumentLoaderRegistry.register("chemdner",
				"ChemDNER format document sets", new ChemDNER());
		...
		BioCDocumentLoaderRegistry.register("sli",
				"Single Line Input document sets", new SingleLineInput());
		...
		ResultFormatterRegistry.register("brat",
				"BRAT Annotation format (.ann)", new Brat());
		ResultFormatterRegistry.register("mmi",
				"Fielded MetaMap Indexing-like Output", new MMI());
		ResultFormatterRegistry.register("cuilist", "UMLS CUI List Output",
				new CuiList());

		/**
		 * augment or override any built-in formats with ones specified by
		 * property file.
		 */
		BioCDocumentLoaderRegistry.register(properties);
		ResultFormatterRegistry.register(properties);

		this.setSemanticGroup(properties.getProperty(
				"metamaplite.semanticgroup", "all").split(","));
		this.setSourceSet(properties
				.getProperty("metamaplite.sourceset", "all").split(","));
		System.setProperty(
				"metamaplite.result.formatter.property.brat.typename",
				properties.getProperty(
						"metamaplite.result.formatter.property.brat.typename",
						"metamaplite"));
		this.detectNegationsFlag = Boolean.parseBoolean(properties.getProperty(
				"metamaplite.detect.negations", "true"));
		this.setSegmentationMethod(properties.getProperty(
				"metamaplite.segmentation.method", "SENTENCE"));
	}
	 ...
	 *            list of semantic type strings
	 */
	public void setSemanticGroup(String[] semanticTypeList) {
		this.semanticGroup = new HashSet<String>(
				Arrays.asList(semanticTypeList));
	}
	 ...
	 * 
	 * @param typeName
	 *            name of segmentation method to use
	 */
	...
	public BioCSentence processSentence(BioCSentence sentence,
			BioCPassage passage) throws IllegalAccessException,
			InvocationTargetException, IOException, Exception {
		logger.debug("enter processSentence");
		// BioCSentence annotatedSentence =
		// SentenceAnnotator.tokenizeSentence(passage, sentence);
		BioCSentence result0 = sentenceAnnotator.addEntities(this.entityLookup,
				sentence, passage);
	 ...
	 * 
	 * @param passage
	 *            containing list of sentences
	 * @return list of results from sentence processing pipeline, one per
	 *         sentence in input list.
	 */
	...
	public List<Entity> processPassage(BioCPassage passage)
		...
		case LINES:
			...
			for (String segment : segmentList) {
				...
				offset = segment.length(); // preserve offsets even for blank
											// lines.
			}
			passage0 = passage;
			break;
		...
		// BioCPassage passageWithSentsAndAbbrevs =
		// abbrConverter.getPassage(passage0);
		...
		for (BioCAnnotation note : passage0.getAnnotations()) {
			passageWithSentsAndAbbrevs.addAnnotation(abbrConverter
					.getAnnotation(note));
		}
		...
		logger.info("passage relations: "
				+ passageWithSentsAndAbbrevs.getRelations());
		logger.info("passage annotations: "
				+ passageWithSentsAndAbbrevs.getAnnotations());
		// BioCPassage newPassage =
		// processSentences(passageWithSentsAndAbbrevs);
		List<Entity> entityList = MarkAbbreviations.markAbbreviations(
				passageWithSentsAndAbbrevs, this.entityLookup.processPassage(
						(passage.getInfon("section") != null) ? passage
								.getInfon("section") : "text",
						passageWithSentsAndAbbrevs, this.detectNegationsFlag,
						this.semanticGroup, this.sourceSet));
	...
	public List<Sentence> getSentenceList(List<BioCDocument> documentList) {
		...
		for (BioCDocument document : documentList) {
			for (BioCPassage passage : document.getPassages()) {
				sentenceList.addAll(SentenceExtractor.createSentenceList(
						passage.getText(), passage.getOffset()));
			}
		}
		return sentenceList;
	}
	...
	public List<AbbrInfo> getAcronymList(List<BioCDocument> documentList) {
		...
		for (BioCDocument document : documentList) {
			for (BioCPassage passage : document.getPassages()) {
				for (Sentence sentence : SentenceExtractor
						.createSentenceList(passage.getText())) {
					infos.addAll(extractAbbr.extractAbbrPairsString(sentence
							.getText()));
				}
			}
		}
	...
	static void displayHelp() {
		...
		System.err
				.println("  --              Read from standard input, write to standard output");
		System.err
				.println("  --pipe          Read from standard input, write to standard output");
		...
		System.err
				.println("  --segmentation_method=SENTENCES|BLANKLINES|LINES    set method for text segmentation");
		System.err
				.println("  --segment_sentences    Set method for text segmentation to sentences");
		System.err
				.println("  --segment_blanklines   Set method for text segmentation to each blanklines");
		System.err
				.println("  --segment_lines        Set method for text segmentation to each line");
		System.err
				.println("  --usecontext           Use ConText negation algorithm.");
		System.err
				.println("  --enable_postagging=[true|false]  Use part-of-speech tagging (default: true).");
	...
	public static void expandModelsDir(Properties properties, String modelsDir) {
		if (modelsDir != null) {
			properties.setProperty("opennlp.en-sent.bin.path", modelsDir
					+ "/en-sent.bin");
			properties.setProperty("opennlp.en-token.bin.path", modelsDir
					+ "/en-token.bin");
			properties.setProperty("opennlp.en-pos.bin.path", modelsDir
					+ "/en-pos-maxent.bin");
		}
	}
	...
	public static void expandIndexDir(Properties properties, String indexDirName) {
		if (indexDirName != null) {
			properties.setProperty("metamaplite.ivf.cuiconceptindex",
					indexDirName + "/indices/cuiconcept");
			...
			properties.setProperty("metamaplite.ivf.cuisourceinfoindex",
					indexDirName + "/indices/cuisourceinfo");
			properties.setProperty("metamaplite.ivf.cuisemantictypeindex",
					indexDirName + "/indices/cuist");
			properties.setProperty("metamaplite.ivf.varsindex", indexDirName
					+ "/indices/vars");
		}
	}

	public static void expandIndexDir(Properties properties) {
		String indexDirName = properties
				.getProperty("metamaplite.index.directory");
	...
	public static Properties getDefaultConfiguration() {
		String modelsDirectory = System.getProperty("opennlp.models.directory",
				"data/models");
		String indexDirectory = System.getProperty(
		...
		defaultConfiguration.setProperty("metamaplite.index.directory",
				indexDirectory);
		defaultConfiguration.setProperty("metamaplite.document.inputtype",
				"freetext");
		...
		defaultConfiguration.setProperty("metamaplite.segmentation.method",
				"SENTENCES");
		...
		defaultConfiguration.setProperty(
				"metamaplite.ivf.cuisemantictypeindex", indexDirectory
						+ "/strict/indices/cuist");
		...
		defaultConfiguration
				.setProperty("bioc.document.loader.sldiwi",
						"gov.nih.nlm.nls.metamap.document.SingleLineDelimitedInputWithID");

		defaultConfiguration.setProperty(
				"metamaplite.result.formatter.cuilist",
	...
	static Properties setConfiguration(String propertiesFilename,
			Properties defaultConfiguration, Properties systemConfiguration,
			Properties optionsConfiguration, boolean verbose)
		...
		if (loader == null)
			loader = ClassLoader.getSystemClassLoader(); // use system class
															// loader if class
															// loader is null
		...
		try {
			...
			logger.info("Could not load configuration file from classpath: "
					+ propertiesFilename);
		}
		...
		if (localConfigurationFile.exists()) {
			logger.info("loading local configuration from "
					+ localConfigurationFile);
			if (verbose) {
				System.out.println("loading local configuration from "
						+ localConfigurationFile);
			}
			localConfiguration.load(new FileReader(localConfigurationFile));
			logger.info("loaded " + localConfiguration.size()
					+ " records from local configuration");
			if (verbose) {
				System.out.println("loaded " + localConfiguration.size()
						+ " records from local configuration");
			}
		}
		...
		Properties properties = Configuration.mergeConfiguration(
				defaultConfiguration, localConfiguration, systemConfiguration,
	...
	void listSentences(List<BioCDocument> documentList) {
		...
		for (Sentence sent : this.getSentenceList(documentList)) {
			pw.println(sent.getOffset() + "|" + sent.getText().length() + "|"
					+ sent.getText());
		}
		pw.flush();
	}
	...
	void listEntities(List<BioCDocument> documentList, String outputFormatOption)
		...
		PrintWriter pw = new PrintWriter(new OutputStreamWriter(System.out));
		// format output
		ResultFormatter formatter = ResultFormatterRegistry
				.get(outputFormatOption);
		if (formatter != null) {
			formatter.entityListFormatter(pw, entityList);
		} else {
			System.out
					.println("! Couldn't find formatter for output format option: "
							+ outputFormatOption);
		}
		pw.flush();
		pw.close();
	...
	void listSentences(String filename, List<BioCDocument> documentList)
			throws IOException {
		// output results for file
		// create output filename
	...
	void listAcronyms(String filename, List<BioCDocument> documentList)
			throws IOException {
		String basename = filename.substring(0, filename.lastIndexOf(".")); //
	...
	void listSentencesWithPosTags(String filename,
			List<BioCDocument> documentList) throws IOException {
		// output results for file
		// create output filename
		String basename = filename.substring(0, filename.lastIndexOf(".")); //
		String outputFilename = basename + ".sentences";
	...
	void listEntities(String filename, List<BioCDocument> documentList,
			String outputExtension, String outputFormatOption)
	...
	public static ArrayList<String> get_mesh_1(String text) throws Exception,
			IOException, ClassNotFoundException, InstantiationException,
			NoSuchMethodException, IllegalAccessException,
			InvocationTargetException {
		ArrayList<String> result = new ArrayList<String>();

		MetaMapLite metaMapLiteInst = (MetaMapLite) MetaMapLite.getDefault();
		...
		for (Entity entity : entityList) {
			for (Ev ev : entity.getEvSet()) {
				// System.out.println(ev.getConceptInfo().getCUI() + "|" +
				// ev.getConceptInfo().getPreferredName());
				result.add(ev.getConceptInfo().getCUI() + "|"
						+ ev.getConceptInfo().getPreferredName());
			}
		}
		return result;
	...
	public static ArrayList<String> get_mesh(MetaMapLite metaMapLiteInst,String text
	...
	public static ArrayList<String> get_cuiID(String text) throws Exception,
			IOException, ClassNotFoundException, InstantiationException,
			NoSuchMethodException, IllegalAccessException,
			InvocationTargetException {
		ArrayList<String> result = new ArrayList<String>();
	...
	public static ArrayList<String> get_cuiID_2(String text,
			MetaMapLite metaMapLiteInst) throws Exception, IOException,
			ClassNotFoundException, InstantiationException,
			NoSuchMethodException, IllegalAccessException,
			InvocationTargetException {
		ArrayList<String> result = new ArrayList<String>();
	...
	void logCacheInfo() {
		...
		System.out
				.println("string -> normalized string cache size: "
						+ gov.nih.nlm.nls.metamap.lite.NormalizedStringCache.normalizeStringCache
								.size());
	 ...
	 * <p>
	 * You'll need the model file for the sentence extractor "en-sent.bin" which
	 * can be downloaded from the opennlp project at
	...
	public static void main(String[] args) throws IOException,
		...
		if (args.length > 0) {
			...
			String propertiesFilename = System
					.getProperty("metamaplite.propertyfile",
			...
			while (i < args.length) {
				if (args[i].substring(0, 2).equals("--")) {
					...
					if (fields[0].equals("--") || fields[0].equals("--pipe")) {
						...
						optionsConfiguration.setProperty(
								"metamaplite.log4jconfig", fields[1]);
					} else if (fields[0].equals("--indexdir")) {
						optionsConfiguration.setProperty(
								"metamaplite.index.directory", fields[1]);
					} else if (fields[0].equals("--modelsdir")) {
						optionsConfiguration.setProperty(
								"opennlp.models.directory", fields[1]);
					} else if (fields[0].equals("--specialtermsfile")) {
						optionsConfiguration.setProperty(
								"metamaplite.excluded.termsfile", fields[1]);
					} else if (fields[0].equals("--inputdocformat")
							|| fields[0].equals("--inputformat")) {
						optionsConfiguration.setProperty(
								"metamaplite.document.inputtype", fields[1]);
					} else if (fields[0].equals("--segmentation_method")) {
						optionsConfiguration.setProperty(
								"metamaplite.segmentation.method", fields[1]);
					} else if (fields[0].equals("--segment_sentences")) {
						optionsConfiguration.setProperty(
								"metamaplite.segmentation.method", "SENTENCES");
					} else if (fields[0].equals("--segment_blanklines")) {
						optionsConfiguration
								.setProperty("metamaplite.segmentation.method",
										"BLANKLINES");
						...
						optionsConfiguration.setProperty(
								"metamaplite.document.inputtype", "freetext");
					} else if (fields[0].equals("--outputformat")) {
						optionsConfiguration.setProperty(
								"metamaplite.outputformat", fields[1]);
						optionsConfiguration
								.setProperty(
										"metamaplite.outputextension",
										(outputExtensionMap
												.containsKey(fields[1]) ? outputExtensionMap
												.get(fields[1]) : "out"));
					} else if (fields[0].equals("--brat")
							|| fields[0].equals("--BRAT")) {
						optionsConfiguration.setProperty(
								"metamaplite.outputformat", "brat");
						optionsConfiguration
								.setProperty(
										"metamaplite.outputextension",
										(outputExtensionMap.containsKey("brat") ? outputExtensionMap
												.get("brat") : "brat"));
					} else if (fields[0].equals("--mmi")
							|| fields[0].equals("--mmilike")) {
						optionsConfiguration.setProperty(
								"metamaplite.outputformat", "mmi");
						optionsConfiguration
								.setProperty(
										"metamaplite.outputextension",
										(outputExtensionMap.containsKey("mmi") ? outputExtensionMap
												.get("mmi") : "mmi"));
						...
						optionsConfiguration.setProperty(
								"metamaplite.semanticgroup", fields[1]);
						...
						if (fields.length < 2) {
							System.err.println("missing argument in \""
									+ args[i] + "\" option");
						} else {
							optionsConfiguration.setProperty(
									"metamaplite.inputfilelist", fields[1]);
						}
						...
						if (fields.length < 2) {
							...
							optionsConfiguration.setProperty(
									"metamaplite.inputfilelist.filename",
									fields[1]);
						}
					} else if (fields[0].equals("--list_sentences")) {
						optionsConfiguration.setProperty(
								"metamaplite.list.acronyms", "true");
					} else if (fields[0].equals("--list_acronyms")) {
						optionsConfiguration.setProperty(
								"metamaplite.list.sentences", "true");
					} else if (fields[0].equals("--list_sentences_postags")) {
						optionsConfiguration.setProperty(
								"metamaplite.list.sentences.with.postags",
								"true");
					} else if (fields[0].equals("--output_extension")) {
						optionsConfiguration.setProperty(
								"metamaplite.outputextension", fields[1]);
					} else if (fields[0].equals("--set_property")) {
						optionsConfiguration.setProperty(fields[1], fields[2]);
					} else if (args[i].equals("--verbose")) {
						verbose = true;
					} else if (args[i].equals("--help")) {
						Properties properties = setConfiguration(
								propertiesFilename, defaultConfiguration,
								System.getProperties(), optionsConfiguration,
								verbose);
						BioCDocumentLoaderRegistry.register(properties);
						ResultFormatterRegistry.register(properties);
						displayHelp();
					...
					if (inputFromStdin) {
						System.err
								.println("unexpected filename in command line argument list: "
										+ args[i]);
					} else {
			...
			Properties properties = setConfiguration(propertiesFilename,
					defaultConfiguration, System.getProperties(),
			...
			/**
			 * set any options in properties configuration file and system
			 * properties first
			 */
			BioCDocumentLoaderRegistry.register(properties);

			String documentInputOption = properties.getProperty(
					"metamaplite.document.inputtype", "freetext");
			String outputFormatOption = properties.getProperty(
					"metamaplite.outputformat", "mmi");
			String outputExtension = properties.getProperty(
					"metamaplite.outputextension", ".mmi");

			boolean listSentences = Boolean.parseBoolean(properties
					.getProperty("metamaplite.list.acronyms", "false"));
			boolean listAcronyms = Boolean.parseBoolean(properties.getProperty(
					"metamaplite.list.sentences", "false"));
			boolean listSentencesWithPosTags = Boolean.parseBoolean(properties
					.getProperty("metamaplite.list.sentences.with.postags",
							"false"));

			String inputfileListPropValue = properties
					.getProperty("metamaplite.inputfilelist");
			if (inputfileListPropValue != null) {
				if (filenameList.size() > 0) {
					filenameList.addAll(Arrays.asList(inputfileListPropValue
							.split(",")));
				} else {
					filenameList = Arrays.asList(inputfileListPropValue
							.split(","));
				}
			}
			String inputfileListFilenamePropValue = properties
					.getProperty("metamaplite.inputfilelist.filename");
			if (inputfileListFilenamePropValue != null) {
				if (filenameList.size() > 0) {
					filenameList
							.addAll(loadInputFileList(inputfileListFilenamePropValue));
				} else {
			...
			if (BioCDocumentLoaderRegistry.contains(documentInputOption)) {
				...
				if (docLoader == null) {
					throw new Exception(
							"Fatal: Document loader for input option \""
									+ documentInputOption
			...
			if (inputFromStdin) {
				...
				List<BioCDocument> documentList = docLoader
						.readAsBioCDocumentList(new InputStreamReader(System.in));
				if (listSentences) {
					...
					metaMapLiteInst.listEntities(documentList,
							outputFormatOption);
				}
			} else {
			...
			// register default document loaders and result formatter for help
			// display.
